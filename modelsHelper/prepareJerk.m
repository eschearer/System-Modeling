...
function [map, info, S, R, V] = ...
   prepareJerk(segment, mode, dt, datS, datR, datV, ratio)
...
map = struct('active',false(nS,1),...        % initialize map structure
   'aR',z,'aY',z,'aJ',z,'aP',z,'aW',z,'aD',z,'aDD',z,'aDDD',z,...   % adding aD the indices of derivatives, aDD the indices of second derivatives, aDDD the indices of third derivatives
   'nR',0,'nY',0,'nJ',0,'nP',0,'nW',0,'nD',0,'nDD',0,'nDDD',0);     % nD the number of derivative values, nDD the number of second derivative values, and nDDD the number of third derivative values
...
map.nX = map.nP + map.nW + map.nD + map.nDD + map.nDDD;                    % number of state variables

[map.aD, n] = addressDerivative(sz.*(segment(:,5)==1),n0);
[map.aDD, n] = addressDerivative(sz.*(segment(:,5)==1),n);
[map.aDDD, n] = addressDerivative(sz.*(segment(:,5)==1),n);

%---------- construct state info ----------------------------------------
tmp = [map.aP map.aW map.aD];                       % assemble x-addresses
z = zeros(map.nX,1);
f = false(map.nX,1);                         % initialize info structure
info = struct('type',z,'spatial',z);

for k = 2:nS                                 % loop over segments
   for tp = 1:3                              % loop over type (p,w,d)
      if tmp(k,tp)>0,                        % process if x-data exists
         adr = tmp(k,tp):(tmp(k,tp) + ...
            szj(segment(k,2)) - 1);          % make list of x-addresses

         info.type(adr) = tp;                % type (p=1,w=2,d=3)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% compute addresses of segments' derivative data in global array

function [a, n] = addressDerivative(sz,start)
