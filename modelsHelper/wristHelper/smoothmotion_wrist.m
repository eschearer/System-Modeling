...
filteredWristPosition_global = [ones(forearmStartIndex-1,1)*forearmPos_global(1,:);
    forearmPos_global(forearmStartIndex:forearmEndIndex,:);
    ones(length(time)-forearmEndIndex,1)*forearmPos_global(end,:)];

if plots == 1
    % Plot wrist position
    figure(1);
    plot(filteredWristPosition_global);  
    hold on
    plot(forearmPosition);
    hold off
end
